person_info = csvread('/Users/nikitlonari/Desktop/data.csv');
new_info = csvread('/Users/nikitlonari/Desktop/input_data.csv');
[person_rows, person_columns] = size(person_info);

for j = 1:size(new_info) 
    for i = 1:size(person_info)
        distance(i,j) = sqrt((person_info(i,1)-new_info(j,1))^2+(person_info(i,2)-new_info(j,2))^2+(person_info(i,3)-new_info(j,3))^2);
    end
end
